...
engine = create_engine("sqlite:///manyToMany_data.db",echo=True)
...
class ProxyRelation(Base):
    __tablename__ = 'proxy'
    user_fk = mapped_column(ForeignKey('user.id'),primary_key=True)
    release_fk = mapped_column(ForeignKey('release.id'), primary_key=True)
    def __repr__(self) -> str:
        return f"USER_FK={self.user_fk} | RELEASE_FK={self.release_fk}"
